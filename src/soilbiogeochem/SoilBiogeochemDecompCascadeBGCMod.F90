module SoilBiogeochemDecompCascadeBGCMod
  ...
  use SoilBiogeochemDecompCascadeConType , only : use_soil_nk_shadow, i_atm
...
contains
  ...
  subroutine init_decompcascade_bgc(bounds, soilbiogeochem_state_inst, soilstate_inst )
    ...
    associate(                                                                                     &
         ...
         is_shadow                      => decomp_cascade_con%is_shadow                          , & ! Output: [logical           (:)     ]  TRUE => pool is a shadow pool
      ...
      if (.not. use_fates) then
         ...
         if (use_soil_nk_shadow) then
            ! CWD SHADOW
            i_shadow_cwd = i_cwd + 1
            floating_cn_ratio_decomp_pools(i_shadow_cwd) = .true.
            decomp_cascade_con%decomp_pool_name_restart(i_shadow_cwd) = 'shadow_cwd'
            decomp_cascade_con%decomp_pool_name_history(i_shadow_cwd) = 'SHADOW_CWD'
            decomp_cascade_con%decomp_pool_name_long(i_shadow_cwd) = 'shadow coarse woody debris'
            decomp_cascade_con%decomp_pool_name_short(i_shadow_cwd) = 'SHADCWD'
            is_litter(i_shadow_cwd) = .false.
            is_soil(i_shadow_cwd) = .false.
            is_cwd(i_shadow_cwd) = .true.
            is_shadow(i_shadow_cwd) = .true.
            initial_cn_ratio(i_shadow_cwd) = 90._r8
            initial_stock(i_shadow_cwd) = params_inst%bgc_initial_Cstocks(i_cwd)
            is_metabolic(i_shadow_cwd) = .false.
            is_cellulose(i_shadow_cwd) = .false.
            is_lignin(i_shadow_cwd) = .false.
         endif
      endif
      ...
      if (.not. use_fates) then
         ...
         if (use_soil_nk_shadow) spinup_factor(i_shadow_cwd) = spinup_factor(i_cwd)
      end if
      !som1
      ...
      if (.not. use_fates) then
         ...
         if (use_soil_nk_shadow) then
            i_shadcwd = 11
            decomp_cascade_con%cascade_step_name(i_shadcwd) = 'SHADCWD'
            cascade_donor_pool(i_shadcwd) = i_shadow_cwd
            cascade_receiver_pool(i_shadcwd) = i_atm
         end if
      end if

      ! confirm the receiver pools of transitions from shadow donor pools
      ! are either i_atm, or shadow pools
      do k = 1, ndecomp_cascade_transitions
         if (.not. is_shadow(cascade_donor_pool(k))) cycle
         if (cascade_receiver_pool(k) /= i_atm .and. &
             .not. is_shadow(cascade_receiver_pool(k))) then
            call endrun(msg='ERROR: receiver from a shadow pool must be a shadow pool'//&
                 errMsg(sourcefile, __LINE__))
         end if
      end do
    ...
    associate(                                                           &
     ...
     ! calc ref rate
      ...
      do j = 1,nlevdecomp
         do fc = 1,num_soilc
            ...
            if (.not. use_fates) then
               ...
               if (use_soil_nk_shadow) decomp_k(c,j,i_shadow_cwd) = decomp_k(c,j,i_cwd)
            end if
      ...
      if (.not. use_fates) then
         ...
         if (use_soil_nk_shadow) then
            pathfrac_decomp_cascade(bounds%begc:bounds%endc,1:nlevdecomp,i_shadcwd) = 1.0_r8
            rf_decomp_cascade(bounds%begc:bounds%endc,1:nlevdecomp,i_shadcwd) = 1.0_r8
         end if
      end if
